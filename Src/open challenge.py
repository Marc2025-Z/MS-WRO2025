...
def is_too_close_to_wall(frame, y_threshold=400):
    """
...
while corner_count < MAX_CORNERS:
    ...
    if walls:
        ...
        servo.angle = 0
        print("⬆️ Adjusting and moving forward...")
        while True:
            frame = picam2.capture_array()
            wall_ahead = detect_wall_bottom(frame)
            cv2.imshow("Wall Detection", frame)

            if wall_ahead:
                print("⛔ Wall detected after turn. Stopping.")
                stop()
                break
